...
# SCPN-MIF-CORE — CI workflow inventory
...
class WorkflowPolicy(TypedDict):
    """Complete CI coordinator, category, order, parity, and limit contract."""
    ...
    legacy_job_sha256: dict[str, str]
...
def job_block_sha256(block: str) -> str:
    """Return the stable SHA-256 digest of one raw job block."""
    return hashlib.sha256(block.encode()).hexdigest()


def read_ci_workflow_source() -> str:
    """Return all executable CI jobs in their historical logical order."""
    ...
    for path in ci_workflow_paths(policy)[1:]:
        for job_id, block in job_blocks(path.read_text(encoding="utf-8")).items():
            ...
            blocks[job_id] = block
    coordinator_blocks = job_blocks(coordinator)
