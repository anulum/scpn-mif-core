...
def test_committed_dashboard_aggregates_real_kernels_honestly() -> None:
    ...
    assert dashboard["kernel_count"] == 19
    assert dashboard["group_count"] == 34
    ...
    assert coverage["python"] == 19
    assert coverage["rust"] == 18  # adc_to_spike_quantiser has no Rust surface
