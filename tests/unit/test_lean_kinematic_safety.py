...
def test_root_imports_kinematic_safety_theorem() -> None:
    ...
    assert "import SCPNMIF.KinematicSafety" in LEAN_ROOT.read_text(encoding="utf-8")
...
def test_kinematic_safety_theorem_compiles_with_lake() -> None:
    """Lean accepts the MIF-011 theorem without axioms or sorry."""
    assert KINEMATIC_SAFETY.exists(), "missing MIF-011 Lean theorem file"
    result = subprocess.run(
        ["lake", "env", "lean", str(KINEMATIC_SAFETY)],
        cwd=ROOT,
        check=False,
