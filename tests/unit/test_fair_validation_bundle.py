...
def test_environment_summary_lifts_project_and_benchmark_context() -> None:
    ...
    assert environment["benchmark_dashboard"]["kernel_count"] == 16
    assert environment["benchmark_dashboard"]["group_count"] == 27
    assert environment["benchmark_dashboard"]["python_versions"] == ["3.12.3"]
