...
| MIF-010 | Timing-aware formal property set | RTL + SymbiYosys/`.sby` tasks | machine-checked safety/liveness/timing reports | k-induction over the trigger fabric + fast-veto lane | SymbiYosys (open-source flow) | PARTIAL: open-tool safety/liveness delivered; full 70-property timing-aware tier roadmap (NEU-C.2-gated) |
